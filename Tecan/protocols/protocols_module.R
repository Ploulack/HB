...
protocols_handler <- function(input,
     ...
     observeEvent(input$ok_protocol, {
          ...
          print("Selected Protocol Info before processed plate update in protocols module in observeEvent(input$ok_protocol...) : ")
          print("And now raw...")
          print(selected_prot)
          print("And now only the plates processed url")

          selected_prot %>%
               select(name, tecan_plates_processed_url) %>%
               print()
          ...
          progress_prot_change$inc(.1, str_interp("Adding ${input$set_protocol} experiment to the DB entry."))
          paste0("file id: ", file_container$id()) %>% print()
          ...
          print("Selected Protocol Info in protocols module in observeEvent(input$ok_protocol...) : ")
          print(selected_prot %>%
                     select(name, tecan_plates_processed_url))
          ...
          progress_prot_change$inc(.1, str_interp("Adding plate number ${input$set_plate_nb} to processed plates in HB Experiments sheet."))
          ...
          progress_prot_change$inc(.1, str_interp("Downloading plates tracking CSV."))
          processed_plates_url <- selected_prot[[paste0(tab_name, "_plates_processed_url")]]

          paste0("String used to access the plates processed url: ",paste0(tab_name, "_plates_processed_url")) %>% print()

          print("url of plates processed csv tracking: "); print(processed_plates_url)

          csv_drive_id <- processed_plates_url %>%
               as_id() %>%
          ...
          progress_prot_change$inc(.1, str_interp("Uploading updated plates tracking CSV."))
          drive_update(file = csv_drive_id,
                       media = csv_path)

          #Storing file id before moving the file (not sure if useful)
          file_id <- file_container$id()

          #Move file to protocol folder
          progress_prot_change$inc(.1, str_interp("Moving file to ${input$set_protocol}"))
          move_drive_file(file_container, prot_name = input$set_protocol, tab_name)

          #Set user file selectInput on the same file
          progress_prot_change$inc(.1, str_interp("Updating UI so that file is default."))
          selected(update_selected(input$set_protocol, file_id))
