...
select_file <- function(input,
     ...
     last_input_file <- reactiveVal(NULL)
     #### DEBUG ####
     observeEvent(input$file, {
          cat("input file id ", input$file, "\n")
          if (is.null(input$file)) return()
          last_input_file(input$file)
          cat("last input file id ", last_input_file(), "\n")
     })
     ...
     return(list(
          id = reactive(last_input_file()),
          file_dribble = reactive(container$files %>% filter(id == last_input_file())),
