...
ms_server <- function(input, output, session) {
     ...
     observeEvent(ms$go_file(), {
          ...
          if (!record$entry_exists || record$entry$data[[1]] %>% is_empty()) {
          ...
          else if (record$entry$data[[1]] %>% is_empty()) {
               query <- str_interp('{"_id" : "${ms$id()}"}')
               upd <- str_interp('{"$set":{"data":${ms_dat_json}}}')
               ms_db$update(query, upd)
          }
          else {
               #Merge tags
               ms$tbl(
